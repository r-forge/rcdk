...
to go through \texttt{eval.desc}.\footnote{Note
  that AlogP and XlogP assume that hydrogens are explicitly specified
  in the molecule. This may not be true if the molecules were obtained
  from SMILES}.
...
linear regression model. First, remove NA's, correlated and constant columns.
